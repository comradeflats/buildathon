...
export function GitHubSubmissionForm() {
  ...
  const { themes, addTeam, showToast } = useVoting();
  ...
  const handleFetchRepo = async () => {
    ...
    try {
      ...
      setRepoData(data);
      // Pre-fill team name from repo name
      if (!teamName) {
        setTeamName(data.fullName.split('/')[0] || '');
      }
    } catch (err) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const newTeam: Team = {
      ...
      projectName: repoData.fullName.split('/')[1] || repoData.fullName,
    ...
    addTeam(newTeam);
    showToast(`${newTeam.projectName} added to competition!`, 'success');
    router.push('/');
  };
